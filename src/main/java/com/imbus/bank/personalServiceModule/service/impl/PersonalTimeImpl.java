...
@Service
public class PersonalTimeImpl implements IPersonalTime {
    @Autowired
    ...
    public TimeWithdrawBo timeWithdraw(TimeWithdrawEntity timeWithdrawEntity) {
        ...
        if(timeWithdrawBo.getResult() == TimeWithdrawResult.WITHDRAW_SUCCESS){
            ...
            // 现金账目：减少取款额
            bookCommon.addCashBill(timeWithdrawBo.getAmount().negate(),accountID,"定期取款",false,2);
    ...
    private TimeWithdrawBo getWithdrawBo(String accountID,int depositID,String pwd){
        ...
        TimeDepositItemBo timeDepositItemBo = personalTimeDepositDao.getDeposit(depositID);
        ...
        if(yesterday.getTime() < timeDepositItemBo.getScheduledWithdrawDate().getTime()){
            // 提前支取
            BigDecimal rate = bankConfigDao.getPersonalDemandRate();
            ...
            personalTimeDepositDao.updateTimeDepositStatus(depositID,3);
        }else {
            // 到期支取
            ...
            personalTimeDepositDao.updateTimeDepositStatus(depositID,1);
        }
    ...
    public List<TimeDepositItemBo> getTimeDeposit(String accountID) {
        return personalTimeDepositDao.getDepositList(accountID);
    }
